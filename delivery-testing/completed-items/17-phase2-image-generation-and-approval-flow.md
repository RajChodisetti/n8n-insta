...
- added ImageKit support for generated-image delivery when `IMAGE_HOST_PROVIDER=imagekit`, including persisted ImageKit metadata in `assets.metadata_json`
...
2. Regenerated the post image through the updated asset workflow so the candidate used an ImageKit-backed public JPEG URL
3. Verified the latest asset row had:
   - `provider = openai_gpt_image_1_rehosted_jpeg_imagekit`
   - a public `ik.imagekit.io/...jpg` `storage_url`
   - `rehost_provider = imagekit`
