...
TMP_DIR="$(mktemp -d)"
TEST_SLUG="tmp-phase3-render-dispatch-$(date +%Y%m%d-%H%M%S)"
TEST_TITLE="The Tower That Returned in the Morning Mist"
TEST_CONTENT_ID=""

info() {
  printf '[test_phase3_render_worker_dispatch_smoke] %s\n' "$*"
}

fail() {
  printf '[test_phase3_render_worker_dispatch_smoke] ERROR: %s\n' "$*" >&2
  exit 1
}
...
ensure_queue_is_clear() {
  local queued_rows
  queued_rows="$(psql_query "select content_id, slug, status from content_items where status in ('script_complete', 'storyboarding', 'storyboard_complete', 'generating_assets', 'assets_ready', 'generating_narration', 'narration_ready', 'building_render_manifest', 'render_manifest_ready', 'dispatching_render') order by updated_at asc;")"
...
set nodes = \$phase3dispatchnodes\$$runtime_nodes\$phase3dispatchnodes\$::json,
    connections = \$phase3dispatchconnections\$$runtime_connections\$phase3dispatchconnections\$::json,
    "updatedAt" = now()
...
seed_content_item() {
  ...
  [[ -n "$TEST_CONTENT_ID" ]] || fail "Failed to seed the temporary Phase 3 render-dispatch content item."
}
...
assert_dispatch_result() {
  local row
  row="$(psql_query "select ci.status, r.render_status, coalesce(r.render_log, '') from content_items ci join renders r on r.content_id = ci.content_id where ci.content_id = '$TEST_CONTENT_ID'::uuid;")"
  [[ -n "$row" ]] || fail "Render dispatch workflow did not find a renders row for $TEST_CONTENT_ID."
  IFS=$'\t' read -r content_status render_status render_log <<<"$row"
  [[ "$content_status" == "render_queued" ]] || fail "Expected content status render_queued after dispatch, got $content_status."
  [[ "$render_status" == "queued" ]] || fail "Expected render_status queued after dispatch, got $render_status."
  [[ -n "$render_log" ]] || fail "render_log is empty after dispatch."

  local run_row
  run_row="$(psql_query "select run_status, coalesce(details_json->'render_request'->>'worker_mode', ''), coalesce(details_json->'render_request'->>'scene_count', ''), coalesce(details_json->'render_request'->>'output_path', '') from workflow_runs where content_id = '$TEST_CONTENT_ID'::uuid and workflow_name = 'wf_render_worker_dispatch' order by started_at desc limit 1;")"
  [[ -n "$run_row" ]] || fail "wf_render_worker_dispatch did not write a workflow_runs row."
  IFS=$'\t' read -r run_status worker_mode scene_count output_path <<<"$run_row"
  [[ "$run_status" == "success" ]] || fail "Expected wf_render_worker_dispatch workflow_runs status success, got $run_status."
  [[ -n "$worker_mode" ]] || fail "workflow_runs.details_json.render_request.worker_mode is empty."
  [[ "${scene_count:-0}" -ge 4 ]] || fail "workflow_runs render_request.scene_count is too small."
  [[ -n "$output_path" ]] || fail "workflow_runs render_request.output_path is empty."
}

print_summary() {
  info "Temporary content_id: $TEST_CONTENT_ID"
  info "Temporary slug: $TEST_SLUG"
  psql_query "select ci.status, r.render_status, coalesce(r.render_log, '') from content_items ci join renders r on r.content_id = ci.content_id where ci.content_id = '$TEST_CONTENT_ID'::uuid;" | awk -F'\t' '
    {
      printf "[test_phase3_render_worker_dispatch_smoke] Final state: content_status=%s render_status=%s render_log_present=%s\n",
        $1, $2, ($3 != "" ? "yes" : "no")
    }
  '
}
...
info "Checking the render-dispatch queue is clear before the smoke test"
ensure_queue_is_clear
