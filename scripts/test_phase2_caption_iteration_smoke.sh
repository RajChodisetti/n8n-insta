...
assert_workflow_log() {
  local row
  row="$(psql_query "select run_status, coalesce(details_json ->> 'generation_model', ''), coalesce(details_json ->> 'revision_summary', ''), jsonb_typeof(details_json -> 'caption_iteration') from workflow_runs where content_id = '$TEST_CONTENT_ID'::uuid and workflow_name = 'wf_caption_and_hashtags' order by started_at desc limit 1;")"

  if [[ -z "$row" ]]; then
    fail "Caption workflow did not create a workflow_runs entry for $TEST_CONTENT_ID."
  fi

  IFS=$'\t' read -r run_status generation_model revision_summary caption_iteration_type <<<"$row"
  ...
  [[ -n "$revision_summary" ]] || fail "workflow_runs.details_json.revision_summary is empty."
...
print_summary() {
  ...
  info "Caption workflow completed successfully."
  psql_query "select p.publish_status, length(coalesce(p.caption_final, '')), coalesce(wr.details_json ->> 'generation_model', ''), coalesce(wr.details_json ->> 'revision_summary', '') from publishes p join workflow_runs wr on wr.content_id = p.content_id and wr.workflow_name = 'wf_caption_and_hashtags' where p.content_id = '$TEST_CONTENT_ID'::uuid order by wr.started_at desc limit 1;" | awk -F'\t' '
    {
      printf "[test_phase2_caption_iteration_smoke] Final state: publish_status=%s caption_length=%s generation_model=%s revision_summary=%s\n",
        $1, $2, $3, $4
    }
  '
}
