...
TMP_DIR="$(mktemp -d)"
TEST_SLUG="tmp-phase2-caption-$(date +%Y%m%d-%H%M%S)"
TEST_TITLE="The Lighthouse Keeper Who Vanished Without a Trace"
TEST_CONTENT_ID=""

info() {
  printf '[test_phase2_caption_iteration_smoke] %s\n' "$*"
}

fail() {
  printf '[test_phase2_caption_iteration_smoke] ERROR: %s\n' "$*" >&2
  exit 1
}
...
ensure_queue_is_clear() {
  ...
  if [[ -n "$queued_rows" ]]; then
    fail "Found existing caption-draft queue rows. Clear them before running this smoke test:\n$queued_rows"
  fi
}
...
set nodes = \$captionnodes\$$runtime_nodes\$captionnodes\$::json,
    connections = \$captionconnections\$$runtime_connections\$captionconnections\$::json,
    "updatedAt" = now()
...
seed_content_package() {
  TEST_CONTENT_ID="$(
    psql_query "insert into content_items (
      ...
      35,
      'default',
      '{\"source_notes\":[\"In December 1900, three lighthouse keepers disappeared from the Flannan Isles.\",\"The relief crew found the lighthouse unattended, with signs that the keepers had left suddenly.\",\"The exact cause remains uncertain and is often retold as a maritime mystery.\"]}'::jsonb,
      'storyboard_complete',
      now(),
  ...
  if [[ -z "$TEST_CONTENT_ID" ]]; then
    fail "Failed to seed the temporary Phase 2 caption content item."
  fi
  ...
  psql_query "insert into scripts (
    ...
    'Three keepers. One lighthouse. No clear ending.',
    'The lighthouse was still running, but the men were gone.',
    'One storm. Three disappearances. Zero agreed answers.',
    'The lighthouse was still running, but the men were gone.',
    'When the relief ship reached the Flannan Isles, the lighthouse was working. But all three keepers had vanished, leaving behind one of the sea''s most unsettling mysteries.',
    'The Flannan Isles lighthouse was found active, but its keepers were missing.',
    'The lighthouse was still running, but the keepers were gone.',
    'What do you think happened there?',
    '[{\"scene_number\":1,\"text\":\"The light stayed on.\"},{\"scene_number\":2,\"text\":\"The keepers did not.\"}]'::jsonb,
    'gpt-4o-mini',
    '{}'::jsonb,
    false
  );" >/dev/null
  ...
  psql_query "insert into storyboards (
    ...
    '$TEST_CONTENT_ID'::uuid,
    '[{\"scene_number\":1,\"visual_prompt\":\"Storm over a remote lighthouse at dusk\"}]'::jsonb,
    'Remote lighthouse at dusk, storm building, cinematic mystery poster composition',
    '[\"Three keepers vanished from a working lighthouse.\"]'::jsonb,
    'cold maritime tension',
    '{}'::jsonb
  );" >/dev/null
}
...
assert_caption_result() {
  ...
  [[ -n "$caption_final" ]] || fail "caption_final is empty after caption workflow."
  [[ -n "$hashtags_final" ]] || fail "hashtags_final is empty after caption workflow."
  [[ "${#caption_final}" -ge 40 ]] || fail "caption_final looks too short to be a real iterated caption."
  [[ "$hashtags_final" == *"#"* ]] || fail "hashtags_final does not contain hashtags."
}

assert_workflow_log() {
  local row
  row="$(psql_query "select run_status, coalesce(details_json ->> 'generation_model', ''), coalesce(details_json ->> 'selection_rationale', ''), jsonb_typeof(details_json -> 'caption_iteration'), jsonb_typeof(details_json -> 'hashtag_ranking') from workflow_runs where content_id = '$TEST_CONTENT_ID'::uuid and workflow_name = 'wf_caption_and_hashtags' order by started_at desc limit 1;")"

  if [[ -z "$row" ]]; then
    fail "Caption workflow did not create a workflow_runs entry for $TEST_CONTENT_ID."
  fi

  IFS=$'\t' read -r run_status generation_model selection_rationale caption_iteration_type hashtag_ranking_type <<<"$row"
  ...
  [[ -n "$selection_rationale" ]] || fail "workflow_runs.details_json.selection_rationale is empty."
  [[ "$caption_iteration_type" == "object" ]] || fail "workflow_runs.details_json.caption_iteration is not a JSON object."
  [[ -z "$hashtag_ranking_type" ]] || fail "workflow_runs.details_json.hashtag_ranking should be absent in the single-call caption flow."
}

print_summary() {
  info "Temporary content_id: $TEST_CONTENT_ID"
  info "Temporary slug: $TEST_SLUG"
  info "Single-call caption workflow completed successfully."
  psql_query "select p.publish_status, length(coalesce(p.caption_final, '')), coalesce(wr.details_json ->> 'generation_model', ''), coalesce(wr.details_json ->> 'selection_rationale', '') from publishes p join workflow_runs wr on wr.content_id = p.content_id and wr.workflow_name = 'wf_caption_and_hashtags' where p.content_id = '$TEST_CONTENT_ID'::uuid order by wr.started_at desc limit 1;" | awk -F'\t' '
    {
      printf "[test_phase2_caption_iteration_smoke] Final state: publish_status=%s caption_length=%s generation_model=%s selection_rationale=%s\n",
        $1, $2, $3, $4
    }
  '
}
...
info "Checking the caption queue is clear before the smoke test"
ensure_queue_is_clear
...
run_workflow_inline "$WORKFLOW_ID" "wf_caption_and_hashtags" "$TMP_DIR/caption_execution.json"
