...
- Remotion edit-plan fixtures are planning examples, not a Remotion app or runtime install. Future-only setup commands in fixtures must not be run unless a later Remotion runtime session explicitly asks for it.
- Avatar decision fixtures are consent-gated planning examples, not avatar runtime state. They must not be treated as provider account setup, avatar video generation, publish approval, or consent evidence.
...
- `remotion_edit_plan` can describe Remotion composition data and frame timing, but active rendering remains `local_ffmpeg` until a later wiring session explicitly changes it.
