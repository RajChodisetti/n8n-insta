...
3. Studio enqueues a `pipeline_runs` row and ordered `pipeline_steps` for the default `generate_reel` action, including `reel_type` (`image`, `video`, `avatar`, or `hybrid`) plus a stored `creative_workflow` strategy (`high_retention_story`, `premium_documentary`, or `sales_conversion`). The visible UI keeps creative workflow selection separate from Image Reel, Video Reel, Avatar Reel, and Hybrid / Auto Reel execution buttons.
...
6. Text stages build prompt requests from `prompts/` using `build_prompt_request.mjs`; `prompt_stage_defaults.mjs` injects the selected creative workflow role/few-shot card; all structured text stages share the Studio-selected `TEXT_LLM_PROVIDER`, prompt-generation stages share `TEXT_MODEL` or `TEXT_ANTHROPIC_MODEL`, and only captions/hashtags plus final QA can use separate model overrides; `invoke_structured_text_adapter.mjs` calls the selected OpenAI or Anthropic text provider and returns structured JSON.
