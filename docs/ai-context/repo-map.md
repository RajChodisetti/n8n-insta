...
| `prompts/` | File-backed prompt templates, creative workflow role docs, and response schemas used by workflows and Studio UI. | `prompts/README.md`, `*/system.md`, `*/user.md`, `*/response-schema.json`, `creative_workflows/*.md`, image/narration prompt files | Yes for prompt/model-output work | prompts/source | Preserve `{{placeholder}}` tokens unless updating the supplying code. |
| `pipeline/` | Code-first orchestration worker, reel-type stage plans (`image`, `video`, `avatar`, `hybrid`), run/step/event tracking, and migrated pipeline stage handlers. | `AGENTS.md`, `worker.mjs`, `runs.mjs`, `stages.mjs`, `schema.mjs` | Yes for orchestration work | source/backend | Default generate action must stop at approval. Do not change active `reel_type` mid-run. Do not remove n8n fallback until parity is proven. |
