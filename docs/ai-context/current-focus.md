...
- Creative workflow selection is now separate from reel type for code-first runs: Studio/API stores `creative_workflow` on the topic source payload, and prompt rendering injects the selected role/few-shot card into idea ingest, story package, director, storyboard split, visual prompt, voice performance, and final QA stages.
- Scene-level asset planning is now active: image reels default to `image_with_motion`, video reels generate a `scene_reference_image` keyframe before reference-to-video by default, and v3 asset generation is strict by default for video reels. Set `ALLOW_VIDEO_TO_IMAGE_FALLBACK=true` only when provider-video failures should intentionally degrade into still images plus Remotion motion.
...
- Code-first generation now actively runs `director_contract`, `storyboard_and_shot_plan`, `visual_prompt_builder`, `voice_performance_script` for image/video narration, and `final_qa_validator`; several of these were formerly contract-only prompt assets.
...
- Remotion is now the default code-first renderer through `infra/remotion-renderer/`; the renderer consumes per-scene camera moves, pan/zoom direction, transition type, overlays, pacing, and motion-layer hints, then adds active depth/light/edge motion layers so still assets do not render as bland holds. The existing FFmpeg render worker remains rollback fallback until parity is documented.
- Avatar video support now runs through `avatar_presenter_selector` plus `avatar_media_generation`; missing consent, unsafe direction, or incomplete HeyGen config auto-downgrades the requested avatar run to the existing video reel path instead of failing generation. The fallback path preserves the storyboard's per-scene asset plan, reuses ready `scene_image` or `scene_video` assets, and tags the content as an avatar downgrade so v3 asset generation does not force strict provider-video scenes.
- Studio now keeps creation, approval, provider readiness, pipeline tracking, and reel-detail inspection visible. The curated Runtime Settings form can save provider/model, voice, music, and provider API-key overrides into the repo-root `.env`; blank values fall back to existing env/defaults and masked secrets are preserved. Each reel detail view exposes final media, scene assets, asset-generation prompts, costs, and pipeline evidence. The Avatar Reel form includes an explicit HeyGen avatar/voice consent checkbox that writes per-run avatar policy metadata.
