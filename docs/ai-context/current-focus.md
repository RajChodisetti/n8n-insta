...
- Studio now exposes runtime provider/model/avatar settings plus a collapsed provider API key section; new Node-side LLM/avatar provider calls read the repo-root `.env` through the runtime env overlay instead of requiring a restart for those values. The Avatar Reel form includes an explicit HeyGen avatar/voice consent checkbox that writes per-run avatar policy metadata.
