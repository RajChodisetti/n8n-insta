...
- Avatar video support now runs through `avatar_presenter_selector` plus `avatar_media_generation`; missing consent, unsafe direction, or incomplete HeyGen config auto-downgrades the requested avatar run to the existing video reel path instead of failing generation. The fallback path now promotes storyboard scenes to provider-video plans and reuses only ready `scene_video` assets.
