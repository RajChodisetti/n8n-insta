...
`build_prompt_request.mjs` uses `TEXT_LLM_PROVIDER` as the shared default text provider. Prompt-generation stages share `TEXT_MODEL` / `OPENAI_TEXT_MODEL` or `TEXT_ANTHROPIC_MODEL` / `ANTHROPIC_TEXT_MODEL` / `ANTHROPIC_MODEL`; `caption_and_hashtags` and `final_qa_validator` can use separate provider/model overrides via `CAPTION_LLM_PROVIDER` plus `CAPTION_MODEL` / `CAPTION_ANTHROPIC_MODEL`, and `FINAL_QA_LLM_PROVIDER` plus `FINAL_QA_MODEL` / `FINAL_QA_ANTHROPIC_MODEL`. `adapter_config.mjs` recognizes stage-specific `*_LLM_PROVIDER` variables, but standard runtime setup should keep them blank except for deliberate caption/final-QA exceptions. `invoke_structured_text_adapter.mjs` supports OpenAI and Anthropic/Claude structured text calls; other text providers still throw `providerNotImplemented`.
...
- Scene video generation: Fal Veo 3.1 Fast helper paths in `generate_and_rehost_scene_assets_v3.mjs` and `generate_and_rehost_scene_video.mjs`; v3 generates storyboard reference images first, then animates them with Fal Veo Fast image-to-video by default.
