...
- the current recommended live-post path uses Google Cloud Storage so Meta fetches a normal public object URL instead of your local MinIO URL
...
4. Confirm the delivery URL is public before live publish:
   - `docker exec n8n-insta-postgres psql -U n8n_insta -d n8n_insta -c "select a.storage_url, a.metadata_json->'google_cloud_storage'->>'bucket' as bucket, a.metadata_json->'google_cloud_storage'->>'object_key' as object_key, a.metadata_json->>'rehost_provider' as rehost_provider from assets a join content_items ci on ci.content_id = a.content_id where ci.slug like 'tmp-phase2-live-%' and a.asset_role = 'post_image' order by a.created_at desc limit 1;"`
5. If you changed GCS env values after the last asset run, rerun only the asset workflow before approval:
