...
import {
  ...
  PROMPT_PROFILE_ALLOWED_FIELDS,
  PROMPT_PROFILE_STAGE_KEYS,
} from './prompt_profile_contract.mjs';
...
const WORKFLOW_FILES = {
  wf_story_package_generation: '/workflows/n8n/wf_story_package_generation.json',
  wf_research_and_script: '/workflows/n8n/wf_research_and_script.json',
  wf_director: '/workflows/n8n/wf_director.json',
  wf_storyboard_and_prompts: '/workflows/n8n/wf_storyboard_and_prompts.json',
  wf_asset_generation_v3: '/workflows/n8n/wf_asset_generation_v3.json',
  ...
  wf_render_worker_dispatch_v2: '/workflows/n8n/wf_render_worker_dispatch_v2.json',
  wf_render_sync_completion_v2: '/workflows/n8n/wf_render_sync_completion_v2.json',
...
function buildPromptProfileContract() {
  const contract = {};
  for (const stageKey of PROMPT_PROFILE_STAGE_KEYS) {
    contract[stageKey] = {};
    for (const field of PROMPT_PROFILE_ALLOWED_FIELDS[stageKey]) {
      contract[stageKey][field] = {
        label: titleCaseFromSnakeCase(field),
        description: `Runtime placeholder value for ${stageKey}.${field}.`,
        examples: [],
      };
    }
  }
...
function buildPromptProfileSeed(candidate) {
  const sourcePayload = candidate.source_payload_json ?? {};
  const abstractIdea = String(sourcePayload.abstract_idea || '').trim();
  if (abstractIdea) {
    return abstractIdea;
  }

  const summary = String(sourcePayload.summary || '').trim();
  const context = String(sourcePayload.context || '').trim();
  const notes = normalizeStringArray(sourcePayload.notes);
  const parts = [
...
function buildGeneratedTopicPayload(candidate) {
  const sourcePayload = candidate.source_payload_json ?? {};
  return {
    title: String(candidate.title || '').trim(),
    category: String(candidate.category || 'general').trim() || 'general',
    confidence_label: String(candidate.confidence_label || 'unverified').trim() || 'unverified',
    target_duration_seconds: Number(candidate.target_duration_seconds || 45),
    summary: String(sourcePayload.summary || '').trim(),
    notes: normalizeStringArray(sourcePayload.notes),
    source_urls: normalizeStringArray(sourcePayload.source_urls),
    context: String(sourcePayload.context || '').trim(),
  };
}

async function refreshPromptProfileForCandidate(client, candidate) {
  const abstractIdea = buildPromptProfileSeed(candidate);
  if (!abstractIdea) {
    fail(`Prompt-profile refresh requires abstract idea context for ${candidate.slug || candidate.content_id}.`);
  }

  const generatedPayload = buildGeneratedTopicPayload(candidate);
  const result = await invokeStructuredTextStage('idea_prompt_profile', {
    abstract_idea: abstractIdea,
    prompt_template_data: {
      abstract_idea: abstractIdea,
      generated_topic_payload_json: JSON.stringify(generatedPayload, null, 2),
      prompt_profile_contract_json: JSON.stringify(buildPromptProfileContract(), null, 2),
    },
  });
  const promptProfile = normalizePromptProfile(result.idea_prompt_profile_response ?? {});
  if (!promptProfile.profile_summary) {
    fail(`Prompt-profile refresh returned an empty profile_summary for ${candidate.slug || candidate.content_id}.`);
  }
  const hasStageOverrides = PROMPT_PROFILE_STAGE_KEYS.some(
    (stageKey) => Object.keys(promptProfile[stageKey] ?? {}).length > 0,
  );
  if (!hasStageOverrides || !hasPromptProfileOverrides(promptProfile)) {
    fail(`Prompt-profile refresh returned no runtime placeholder overrides for ${candidate.slug || candidate.content_id}.`);
  }

  const nextSourcePayload = {
    ...(candidate.source_payload_json ?? {}),
    abstract_idea: String(candidate.source_payload_json?.abstract_idea || '').trim() || abstractIdea,
    prompt_profile_summary: promptProfile.profile_summary,
    prompt_profile: Object.fromEntries(
      PROMPT_PROFILE_STAGE_KEYS.map((stageKey) => [stageKey, promptProfile[stageKey] ?? {}]),
    ),
    prompt_profile_generation_provider: String(result.generation_provider || result.llm_provider || 'openai').trim() || 'openai',
    prompt_profile_generation_model: String(result.generation_model || '').trim(),
    prompt_profile_provider_metadata: result.provider_metadata ?? {},
    prompt_profile_refreshed_at: new Date().toISOString(),
  };

  await client.query(
    `update content_items
    set source_payload_json = $2::jsonb,
        updated_at = now()
    where content_id = $1`,
    [candidate.content_id, JSON.stringify(nextSourcePayload)],
  );

  candidate.source_payload_json = nextSourcePayload;
  return {
    abstract_idea: abstractIdea,
    prompt_profile_summary: promptProfile.profile_summary,
    generation_provider: nextSourcePayload.prompt_profile_generation_provider,
    generation_model: nextSourcePayload.prompt_profile_generation_model,
  };
}
...
function buildStagePlan(candidate) {
  const renderStage = useSyncRenderPath() ? 'wf_render_sync_completion_v2' : 'wf_render_worker_dispatch_v2';
  const shouldWaitForRender = !useSyncRenderPath();
  switch (candidate.content_status) {
    case 'idea_approved':
      return [
        'wf_story_package_generation',
...
function assertStageOutcome(stage, summary) {
  if (stage === 'wf_story_package_generation') {
    if (summary.content_status !== 'storyboard_complete') {
      fail(
  ...
  if (stage === WAIT_FOR_RENDER_STAGE || stage === 'wf_render_sync_completion_v2') {
    if (
...
async function main() {
  ...
  try {
    ...
    const promptProfileRefresh = null;
    ...
    if (planOnly) {
      process.stdout.write(`${JSON.stringify({
        ...
        prompt_profile_refresh: 'skipped for v3 story-package flow',
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      normalized_resume_status: candidate.content_status,
      render_strategy: useSyncRenderPath() ? 'sync' : 'async',
      prompt_profile_refresh: promptProfileRefresh,
